...
export default function Debug() {
  ...
  const [currentMode, setCurrentMode] = React.useState<string>("debug");
    ...
    return (
      ...
      <Container>
        <Box>
          ...
          <Box>
            <CustomButton onClick={() => setCurrentMode("styleguide")}>
              Load Styleguide
            </CustomButton>
          </Box>
          {/* System status */}
          <Box my="8">
            ...
            >
              <Box bgColor={"gray.100"}>
                User status :{" "}
                {auth?.firebaseAuthState.isLoggedIn
                  ? "Logged In"
                  : "Logged Out"}
              </Box>
  ...
  } else if (currentMode === "styleguide") {
    return <StyleGuide />;
  }
}
